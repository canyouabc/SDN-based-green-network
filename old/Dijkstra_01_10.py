...
# 新增功能：透過狀態，更新 Dijkstra 路徑計算，並且使特定 switch 關閉，以達到節能的效果
# 為達到上述功能，需要有更新判斷 Dijkstra 路徑的機制
# 以及能夠控制 switch 狀態的功能
...
class ProjectController(app_manager.RyuApp):
            ...
            for switch_a in list(self.temp_adjacency.keys()):
                for switch_b in list(self.temp_adjacency[switch_a].keys()): 
                  ...
                  if switch_a >= switch_b:
                    thread = hub.spawn(self._switch_to_switch_delay_count, switch_a, switch_b)
    ...
    def dynamic_Dijkstra_test(self):
        ...
        while True:
            ...
            green_Dijkstra = "enable"  # "enable" 或 "disable"，到時候可以設定開關，現在測試而已，先寫死
                    ...
                    if host_a != host_b:
                        ...
                        if (host_a, host_b) in temp_route:
                            if p == temp_route[(host_a, host_b)]:
                                self.install_flows_for_path(p, host_a, host_b, 1, 30)
                            else:
                                print(f"Path change detected for {host_a} -> {host_b}")
                                self.install_flows_for_path(p, host_a, host_b, 2, 12)
    ...
    def _switch_to_switch_delay_count(self, switch_a, switch_b):
        ...
        for i in range(3):
          ...
          temp_echo_link_delay[(switch_a)] = 0
          temp_echo_link_delay[(switch_b)] = 0
          ...
          while (temp_lldp_link_delay[(switch_a, switch_b)] <= 0 or temp_echo_link_delay[(switch_a)] <=0
                or temp_lldp_link_delay[(switch_b, switch_a)] <= 0 or temp_echo_link_delay[(switch_b)] <=0):
            hub.sleep(0.1)
            sleep_time += 0.1
            if (sleep_time >= 1):
              if i >= 2:
                print(f"*** Timeout measuring delay between {switch_a} and {switch_b}")
                delay = 99999  # timeout
                break
              else:
                print(f" retry measure delay between {switch_a} and {switch_b}")
                break
          if (sleep_time >= 1):
            hub.sleep(0.5)
            continue
          delay = (temp_lldp_link_delay[(switch_a, switch_b)] 
                  ...
                  - temp_echo_link_delay[(switch_a)] 
                  - temp_echo_link_delay[(switch_b)]) / 2
        update_link_delay(switch_a, switch_b, delay)
        update_link_delay(switch_b, switch_a, delay)
        
        if delay != 99999:
          print(f"Measured delay between {switch_a} and {switch_b}: {delay} ms")
    ...
    def _send_echo_request(self, switch):
        ...
        temp_echo_link_delay[switch] = temp_echo_link_delay[switch] - (time.time() * 1000)
    ...
    def echo_reply_handler(self, ev):
        ...
        temp_echo_link_delay[dpid] = temp_echo_link_delay[dpid] + (time.time() * 1000)
