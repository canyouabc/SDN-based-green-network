...
### 路徑有一個權重，啟動能耗又是一個權重，不確定參考論文的權重是什麼
### 已啟動的 switch 與 link 權重為 0，未啟動的 switch 與 link 權重為預設的耗能
### 不知道關於 table 1, 論文中的 line card 分別對照幾個 port 
# 條件有三個
# 1. 通過流量需小於 link 的bw
# 2. 每個流量都需被完整的路由
# 3. 只要link上有流量，就必須啟動該link以及相關的switch (反之，若一個switch相鄰的link都沒有流量了，那麼這個switch就關閉)
# 先比較能耗，若能耗相同，再比較路徑長度
        ...
        with open(filepath, 'r') as f:
            for line in f:
                ...
                if not line or line.startswith('#'):
                    continue
                parts = line.split()
                src, dst, value = int(parts[0]), int(parts[1]), int(parts[2])
                # 格式 src_dpid dst_dpid value
                data[(src, dst)] = value
                data[(dst, src)] = value  # 雙向
        print(f"*** {filepath} 載入成功，共 {len(data)//2} 條 link")
    except FileNotFoundError:
        print(f"*** 找不到 {filepath}")
    return data
...
def calculate_energy_saving():
    # 目前能耗（歸0的代表已使用/關閉）
    # 總計為現在"未"使用的 switch 與 link 的能耗總和
    unused_switch_energy = sum(switch_energy.values())
    unused_link_energy   = sum(v for (src, dst), v in link_energy.items() if src < dst)  # 避免雙向重複

    # 如果全開的話，能耗為總和
    total_switch_energy = sum(initial_switch_energy.values())
    total_link_energy   = sum(v for (src, dst), v in initial_link_energy.items() if src < dst) # 一樣避免雙向重複

    unused_total_energy = unused_switch_energy + unused_link_energy
    total_energy = total_switch_energy + total_link_energy

    ratio = ( unused_total_energy / total_energy * 100) if total_energy > 0 else 0
    
    # 顯示已啟動的 Switch
    closed_switches = [dpid for dpid, v in switch_energy.items() if v == 0]
    if closed_switches:
        print(f"  已啟動 Switch：{sorted(closed_switches)}")

    # 顯示已啟動的 Link
    closed_links = [(src, dst) for (src, dst), v in link_energy.items() if v == 0 and src < dst]
    if closed_links:
        print(f"  已啟動 Link：{sorted(closed_links)}")
    print(f"\n===== 能耗統計 =====")
    print(f"  完全總能耗：{total_energy} W")
    print(f"  目前總能耗：{total_energy - unused_total_energy} W")
    print(f"  節省能耗：  {unused_total_energy} W ({ratio:.1f}%)")
    print("====================\n")
...
def get_min_delay_path(src, dst, first_port, final_port, green_adjacency=None):
    ...
    while len(Q) > 0:
        ...
        ### 魔改版本：不計延遲，僅計 hop 數，以及關閉特定 switch 的能耗
