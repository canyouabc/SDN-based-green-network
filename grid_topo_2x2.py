...
# vertically to their neighbors. Every switch in this size is on the
# border, so each switch connects to exactly one host.
#
# Host numbering order: top row left→right, right column top→bottom,
# bottom row right→left, left column bottom→top.
# Total hosts = 4 (N=2 special case: all 4 switches are corners).
#
